"""Test AniList Poster Template with timing metrics"""
import sys
import time
...
def test_anilist_poster(query: str = "steins gate", media_type: str = "ANIME"):
    """Test AniList Poster template with detailed timing"""
    print(f"\n{Fore.CYAN}═══ AniList Poster Template Test ═══{Style.RESET_ALL}\n")
    ...
    # Step 4: Save
    start = time.time()
    output = Path("output") / f"anilist_poster_{media_type}_{query.replace(' ', '-')}.png"
...
if __name__ == "__main__":
    ...
    query = " ".join(args) if args else "steins gate"
    test_anilist_poster(query, media_type)
