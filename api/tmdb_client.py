"""
TMDB API Client (Bot side)
==========================
Pure Python client for The Movie Database API used directly by the Telegram
bot to generate movie / TV posters.

This module intentionally has **no** Flask / web-server dependency so the bot
can import it safely. The Flask micro-service (`api/tmdb_service.py`) re-uses
the same :func:`get_tmdb_media` function to expose an HTTP endpoint.
"""
...
# ── Project imports (single source of truth in root config.py) ──
...
def tmdb_get(path: str, params: dict = None, api_key: str = None):
    ...
    Args:
        ...
        api_key: Optional v3 API key to override the configured token.
...
def list_to_str(data_list: list, limit: int = 10, key: str = None):
    """Convert a list of dicts/strings to a comma-separated string."""
...
def extract_title_and_year(query: str):
    """Extract ``(title, year)`` from a query like ``'Inception 2010'``."""
...
def fetch_media_details(media_type: str, media_id: int, api_key: str = None):
    """Fetch full movie/TV details incl. credits, images, external IDs."""
...
def search_media_id(query: str, api_key: str = None):
    """
    Search for the best matching media (movie or TV) using fuzzy matching
    and year filtering. Returns ``(media_type, media_id)`` or ``(None, None)``.
    """
...
def process_images(images_data):
    """Group poster / backdrop URLs by language for flexible poster design."""
...
def search_media_candidates(query: str, limit: int = 5, api_key: str = None) -> list:
    """
    Return the top ``limit`` TMDB search matches as lightweight dicts so the
    bot can show them as a "did you mean…?" selection list.

    Each item contains:
        media_type, media_id, title, year, poster_url, backdrop_url, overview

    Used by the thumbnail generator's callback-picker flow.
    """
    title, year = extract_title_and_year(query)
    params = {"query": title, "language": "en-US", "page": 1, "include_adult": False}
...
def _build_media_data(media_type: str, media_id: int, query: str = "",
                      api_key: str = None) -> dict:
    """Shared builder used by both :func:`get_tmdb_media` and
    :func:`get_tmdb_media_by_id`."""
...
def get_tmdb_media_by_id(media_type: str, media_id: int, api_key: str = None) -> dict:
    """Fetch a clean media-data dict for a known TMDB ``media_type``/``media_id``."""
    return _build_media_data(media_type, media_id, query="", api_key=api_key)


def get_tmdb_media(query: str, api_key: str = None) -> dict:
    """
    Search TMDB for ``query`` and return a clean dictionary of media data
    ready for the poster templates. Returns ``None`` when not found.
    """
